...
class RabbitMQProvider extends Tool {
  ...
  /**
   * Process a consumed/got message:
   ...
   * @private
   */
  _processMsg(msg, save, channel) {
    return main._processMsg(msg, save, channel, this);
  }
   ...
   */
  _connect(force) {
    return main._connect(force === true, this);
  }
   ...
   * @param {function} params.cb - callback function to run after receiving a message, it takes the received message as param
   * @param {string} params.ack - ack mode:
   * - if 'auto': ack as soon as the message is consumed
   * - if 'resolve': ack as soon as the callback is resolved
   * - else you should ack manually by calling provider's ack method
