      ...
      return new Promise((resolve, reject) => {
        co(function * () {
          if (vp.buffer === 'memory' || vp.buffer === 'file') {
            ...
            const aData = yield channel.assertExchange(vp.exchange, 'topic', {durable: 'true'});
            if (channel.publish(vp.exchange, vp.topic, that._jsonToBuffer(vp.json), {persistent: 'true'})) {
              that.logger.debug(`Published new message in exchange '${vp.exchange}' with routing key '${vp.topic}'`, vp.json);
              resolve(aData);
            } else {
              that.logger.error(`Message with routing key '${vp.topic}' not published in exchange '${vp.exchange}'`, vp.json);
              reject(aData);
            }
            channel.close();
          }
