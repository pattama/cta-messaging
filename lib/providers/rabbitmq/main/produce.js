      ...
      return new Promise((resolve, reject) => {
        try {
          if (vp.buffer === true) {
            ...
            if (!that.buffer.intervals.queues) {
              that.buffer.intervals.queues = setInterval(() => {
                for (const queue in that.buffer.queues) {
                  if (!that.buffer.queues.hasOwnProperty(queue)) {
                    continue;
                  }
                  const len = that.buffer.queues[queue].length;
                  if (len > 0) {
                    const messages = _.cloneDeep(that.buffer.queues[queue]);
                    that.buffer.queues[queue] = [];
                    that.produce({
                      queue: queue,
                      json: {
                        messages: messages,
